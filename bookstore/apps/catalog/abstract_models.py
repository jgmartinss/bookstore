...
class Product(TimeStampedModel):
    ...
    special_price = models.DecimalField(
        _("Special price"), max_digits=10, decimal_places=2
    )
    special_price_from_date = models.DateField(
        _("Special price from date"), blank=True, null=True
    )
    ...
    inventory_maintenance_unit = models.PositiveIntegerField(
        _("Inventory maintenance unit"), default=1
    )
    quantity_out_of_stock = models.PositiveIntegerField(
        _("Quantity for item status switch to out of stock"), default=1
    )
    maximum_quantity_in_the_shopping_cart = models.PositiveIntegerField(
        _("MAX/Quantity in the shopping cart"), default=1
    )
    notify_when_stock_is_exhausted = models.BooleanField(
        _("Notify when stock is exhausted?"), default=True
    )
